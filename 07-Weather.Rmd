# Regressions using yield and temperature data

*The fifth assignment had students using PRISM daily data using linear and multiple regression to analyze different variables related to crop yields in Iowa.* 
...
```{r yield download, warning=FALSE, include=FALSE}
...
```{r Winneshiek linear, warning=F,message=F}
...
```{r Winneshiek quadratic, warning=F,message=F}
...
```{r Winneshiek temp, warning=F,message=F}
...
```{r temp v yield all, warning=F,message=F}
...
```{r clinton yield, warning=F,message=F}
#str(soyyieldsall)
